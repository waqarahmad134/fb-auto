...
async function finishUploadSession(pageId, accessToken, videoId, description) {
 ...
 */
export async function postFacebook({ videoPath, content }) {
  if (config.dryRun) {
  ...
  const { videoId, uploadUrl } = await startUploadSession(pageId, pageAccessToken);
  logger.info({ videoId }, "step 08: Facebook upload session started");

  await uploadVideoBinary(uploadUrl, pageAccessToken, videoPath);
  logger.info({ videoId }, "step 08: Facebook binary uploaded");

  await finishUploadSession(pageId, pageAccessToken, videoId, content.description);
